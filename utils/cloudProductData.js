...
class CloudProductData {
  ...
  normalizeProducts(products) {
    ...
    return products.map((product, index) => {
      try {
        ...
        // 处理视频URL - 支持 videoUrl 或 video 字段
        const videoUrl = product.videoUrl || product.video;
        if (videoUrl && typeof videoUrl === 'string' && videoUrl.trim()) {
          normalized.videoUrl = this.cleanFileId(videoUrl.trim());
        }
  ...
  correctCloudFileId(fileId) {
    ...
    // 检查是否已经是正确格式
    if (fileId.startsWith(`cloud://${correctEnvId}/`)) {
      return fileId;
    }
    ...
    const wrongEnvPatterns = [
      ...
      /cloud:\/\/636c-cloud1-7gm53wok768268c9(?!\.636c)/g,
      /cloud:\/\/cloud1-7gm53wok768268c9(?!\.636c)/g  // 修正简化格式
    ];
    
    let correctedUrl = fileId;
    ...
    if (wasModified) {
      console.log('🔧 修正图片文件ID:', fileId, '->', correctedUrl);
    }
  ...
  async getTempFileUrls(products) {
    try {
      ...
      products.forEach(product => {
        ...
        const videoUrl = product.videoUrl || product.video;
        if (videoUrl && typeof videoUrl === 'string' && videoUrl.startsWith('cloud://')) {
          // 🆕 修正文件ID格式后再收集
          const correctedUrl = this.correctCloudFileId(videoUrl);
          allFileIds.add(correctedUrl);
        }
      });
      ...
      const productsWithTempUrls = products.map(product => {
        ...
        // 🔧 修复：处理图片 - 保持云函数返回的 images 数组，只转换为临时URL
        if (product.images && Array.isArray(product.images)) {
          // 如果云函数已经返回了 images 数组，使用它并转换为临时URL
          updatedProduct.images = product.images
            .map(fileId => {
              // 🆕 修正文件ID后再查找临时URL
              const correctedFileId = this.correctCloudFileId(fileId);
              return tempUrlMap[correctedFileId] || fileId; // 优先使用临时URL，回退到原始fileID
            })
            .filter(url => url);
        } else if (product.imageUrls && Array.isArray(product.imageUrls)) {
          // 回退逻辑：如果没有 images 数组，使用 imageUrls
          updatedProduct.images = product.imageUrls
            .map(fileId => {
              // 🆕 修正文件ID后再查找临时URL
              const correctedFileId = this.correctCloudFileId(fileId);
              return tempUrlMap[correctedFileId] || fileId;
            })
            .filter(url => url);
        } else {
        ...
        // 处理视频 - 支持 videoUrl 或 video 字段
        const videoUrl = product.videoUrl || product.video;
        if (videoUrl) {
          // 🆕 修正文件ID后再查找临时URL
          const correctedVideoId = this.correctCloudFileId(videoUrl);
          // 优先使用临时URL，如果没有则使用原始URL
          updatedProduct.videoUrlTemp = tempUrlMap[correctedVideoId] || videoUrl;
          // 保留原始字段
          if (product.videoUrl) {
            updatedProduct.videoUrl = product.videoUrl;
          }
          if (product.video) {
            updatedProduct.video = product.video;
          }
        }
